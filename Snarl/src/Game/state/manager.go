...
func GameManager(gameLevels []level.Level, // The level struct for the first level
	...
	makeAdversaries := func() []actor.Actor {
		var adversaries []actor.Actor
		// zombies
		for i := 0; i < (levelNumber / 2) + 1; i++ {
			adversaries = append(adversaries, actor.NewAdversaryActor(actor.ZombieType, fmt.Sprintf("game_zombie%d", i), 1))
		}
		// ghosts
		for i := 0; i < (levelNumber - 1) / 2; i++ {
			adversaries = append(adversaries, actor.NewAdversaryActor(actor.GhostType, fmt.Sprintf("game_ghost%d", i), 1))
		}
		return adversaries
	}

	var adversaries = makeAdversaries()
	...
	for _, adversary := range state.Adversaries {
		...
		if adversary.Type == actor.GhostType {
		...
		for _, client := range playerClients {
			...
			if IsLevelEnd(*state) {
				...
				if IsGameEnd(*state, numLevels) {
					...
					state.Adversaries = make([]actor.Actor, 0)
					placeActors(state, makeAdversaries(), getBottomRightUnoccupiedWalkable, state.Level.Size)
					continue
				}
			}
